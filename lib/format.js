...
export function timeAgo(timestamp, dic) {
  if (!timestamp) return '';
  const t = dic?.time;
  const diff = Date.now() / 1000 - timestamp;
  const mins = Math.floor(diff / 60);
  if (mins < 60) return t ? t.min(Math.max(mins, 1)) : `${Math.max(mins, 1)} min`;
  const hours = Math.floor(mins / 60);
  if (hours < 24) return t ? t.hours(hours) : `${hours}h`;
  const days = Math.floor(hours / 24);
  if (days < 30) return t ? t.days(days) : `${days}d`;
  const months = Math.floor(days / 30);
  return t ? t.months(months) : `${months}m`;
}

/**
 * Results travel as EA sees them, V for a win, E for a draw, D for a defeat,
 * and those codes are also CSS class names. Only what the reader sees changes
 * with the language, so this translates the letter and nothing else.
 */
export function resultLetter(code, dic) {
  return dic?.results?.[code] || code;
}
