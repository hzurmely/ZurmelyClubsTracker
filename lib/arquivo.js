    ...
    const corpo = JSON.stringify(
      {
        ...
        platform,
        // An empty name means EA did not answer this time. Keeping the one
        // already on disk is what lets a club that EA dropped stay readable.
        name: nomeDoClube || nomeArquivado(platform, clubId),
...
/**
 * The name the archive last saw for a club.
 *
 * It matters when EA stops answering for the club at all: without this the page
 * would have matches to show and nothing to call them.
 */
export function nomeArquivado(platform, clubId) {
  const arquivo = caminhoDoClube(platform, clubId);
  if (!arquivo) return '';
  try {
    if (!fs.existsSync(arquivo)) return '';
    const dado = JSON.parse(fs.readFileSync(arquivo, 'utf8'));
    return typeof dado?.name === 'string' ? dado.name : '';
  } catch {
    return '';
  }
}
